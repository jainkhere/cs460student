...
Robot.prototype.onStep = function () {
  ...
  for (r in all_robots) {
    ...
    if (this.root.position.equals(r.root.position)) continue;

    if (this.root.position.distanceTo(r.root.position) < 50) {
...
Robot.prototype.onAnimate = function () {
  if (this.movement == "raise left arm") {
    ...
    if (this.left_upperleg.quaternion.w < 0.93) {
      ...
      var T = -Math.PI / 4;
      this.left_upperleg.quaternion.slerp(
        new THREE.Quaternion(
          Math.sin(T / 2), // x
          0, // y
          0, // z
